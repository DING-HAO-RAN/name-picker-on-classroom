...
- 窗口使用 `frame: false`，标题栏由渲染器按当前主题自绘（含最小化、最大化/还原、关闭按钮），不使用 Windows 原生标题栏；最大化状态由主进程推送给渲染器，双击标题栏也可最大化。设置抽屉从标题栏下沿开始，不会被窗口外框遮挡。
- 抽取结果会全屏展示，停留时长默认 3 秒，可在设置抽屉中调整（1500 - 10000 毫秒）；旧存档里内置的 1000 毫秒会在加载时迁移为新的默认值。
